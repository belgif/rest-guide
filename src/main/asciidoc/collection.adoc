...
You can filter a collection by specifying the property name as query param. It means that the property value must be equals to the query-param value.

When the same query parameter is given more than once, it means the resources must match one of the value (`OR` operator).

For string query parameters, if you want to allow more flexible filtering (start with, end with fuzy search,...), you have to specify the syntax and the behavior of each query parameter in the documentation.

Examples:

- the API can accept exact strings
- the API can accept a `*` at the beginning and/or at the end and/or in the middle of the string
- the API can always do a "start with" filtering without the need to add a '*' at the end of the string
- the API can accept regular expressions
- the API can accept a `~` at the beginning of the string to do approximative search (trigram search, phonetic search, Levenshtein distance, soundex,... the algorythm used must me specified in the documentation). Please note that the `~` character must be encoded (it is an unsafe character).

.Examples of string filtering
====
 ssin=0000000097       (exact search)
 ssin=\d{2}0229\d{5}   (reg ex; please note that the `\{}` characters must be encoded (they are unsafe characters)
 ssin=000000000*       (start with)
 lastName=*reeth       (end with)
 lastName=~vanreeth    (matches, using phonetic search for example)
====

If you want to filter a collection with an operator other than `=`, you can use the following suffix:
...
.Persons that are younger than 18 years who are ate least 65 years old
====
[subs=normal]
```
GET {API}/persons?query=age=le=17,age=ge=65 HTTP/1.1
```
...
|<<get>>
|/persons
|get all the persons in the collection
...
|name
|query-param
|Filter only persons with a name starting with the specified name.

|age_le
|query-param
|Filter only persons that are at most the specified age old.

|age_ge
|query-param
|Filter only persons that are at least the specified age old.
...
----
​​​{
  "self": "http://rest-reference.test.paas.socialsecurity.be/REST/demo/v1/persons?name=dupont]",
	"items": [{
		"href": "http://rest-reference.test.paas.socialsecurity.be/REST/demo/v1/persons/406798006",
		"title": "Jean Dupont"
	}],
	"total": 1,
}
...
[subs=normal]
```
GET {API}/persons?name=dupon HTTP/1.1​
```
