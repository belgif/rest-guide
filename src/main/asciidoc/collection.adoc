...
- the API can always to a "start with" filtering without the need to add a '*' at the end of the string
...
|name
|query-param
|Filter only employers with a name starting with the specified name.

|totalEmployees_le
|query-param
|Filter only employers that at most x employees.

|totalEmployees_ge
|query-param
|Filter only employers that at least x employees.
...
----
​​​{
  "self": "http://rest-reference.test.paas.socialsecurity.be/REST/demo/v1/companies?name=smal]",
	"items": [{
		"href": "http://rest-reference.test.paas.socialsecurity.be/REST/demo/v1/companies/406798006",
		"title": "Smals"
	}],
	"total": 1,
}
