...
If the schema's `id` is specified, it SHOULD end with the major version number.
====
...
.JSON schema
```YAML
id: http://example.org/person/identification/v1
...
```YAML
"$ref": 'http://example.org/person/identification/v1#/definitions/Ssin'
```
====
