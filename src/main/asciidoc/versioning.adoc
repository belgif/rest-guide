...
In this case, the major version number could be kept the same as only a single version is active at a given time.
...
Use of deprecated elements in requests may be tracked by the server in order to evaluate the impact of their removal.
...
====
The same guidelines apply to sunset an old major version of an API apply as for removal of deprecated elements:
...
* use of the old version may be tracked by the server in order to evaluate the impact of their removal.
====