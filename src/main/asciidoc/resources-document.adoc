...
When designing a new identifier, various requirements may be of importance specific to the use case:
...
* ability generate identifiers at multiple independent sources (e.g. prefix by source-specific identifiers, UUIDs, ...)
...
Each identifier MUST be represented by only one single string value, so that a string equality check can be used to test if two identifiers are identitical. This means that they are case-sensitive and whitespace or leading zeroes are significant.

In the OpenAPI data type for the identifier, a regular expression may be specified if helpful for input validation or hint of the structure (e.g. to avoid whitespace or wrong capitalization), but shouldn't be too restrictive in order to be able to evolve the format. 
...
NOTE: Parts of an identifier may carry some business meaning (like problem type identifiers in this guide), but no application logic should parse and interpret the parts.
