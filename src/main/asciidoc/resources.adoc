...
include::collection.adoc[leveloffset=+1]

=== Controller

A controller resource represents an action other than a standard CRUD-operation.
...
====
Before using a controller resource to represent an action, consider reifying the action as a document (noun) describing the intent of the action.
====

<<long-running-tasks>> are an example of actions modelled as a document instead of a controller.
...
A controller can be represented as a GET operation when it has no side effects:
