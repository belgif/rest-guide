...
More recent https://spec.openapis.org/oas/latest.html[OpenAPI 3.x] versions improve upon OpenAPI 3.0, but SHOULD NOT be used to avoid interoperability problems because they are not yet sufficiently supported by tooling.
====
...
** For inline schemas (i.e. part of another definition) of type `object`, an UpperCamelCase `title` SHOULD be used to provide a human-readable name for code generation. However:
*** if the schema can be reused, it is better to refactor it to its own schema component (<<rule-oas-comp>>).
*** for inline schemas under the `allOf` or `not` keywords, the `title` property is OPTIONAL, because no classes are generated for them.
====
...
[rule#rule-opbody-req]
.Required request payload
====
Request payloads are optional by default in OpenAPI, even though they usually need to be required. It is most of the time an omission if this default wasn't changed.
...
* `Accept-Language`
* `BelGov-Trace-Id` and `BelGov-Related-Trace-Id` (documented in xref:tracing.adoc[])
* `Authorization`
...
As reference data is typically static, consider supporting xref:performance.adoc#caching[caching] headers for these resources.
====
...
====
`additionalProperties` SHOULD be used exclusively to describe an object uniquely representing a map of key-value pairs.
The keys of such maps don't need to respect the naming rules for JSON properties (lowerCamelCase and English).

Other uses of `additionalProperties` (e.g. `additionalProperties: false`) SHOULD be avoided, in order to support API evolution (xref:versioning.adoc#rule-evo-compat[[evo-compat\]]).
====

An example is the description a map of `embedded` resources, as described in xref:performance.adoc#embedding[Embedding].
...
.Unknown input
====
An API SHOULD refuse unknown input, i.e. unknown body (JSON) properties in the payload or unknown query request parameters. A xref:problems/badRequest.adoc#bad-request[ Bad Request] problem response should be returned with an issue of type `urn:problem-type:belgif:input-validation:unknownInput`.
...
When defining a type for an identifier or code, like the above example, the guidelines under xref:resources-document.adoc#section-identifier[Identifier] apply, even when not used as a URL path parameter of a document resource.
...
|===
|Name|Link
|OpenAPI 3.0 specification| https://spec.openapis.org/oas/v3.0
