...
An even more complete list of HTTP status codes is documented https://www.iana.org/assignments/http-status-codes/http-status-codes.xhtml[here^].
...
|xref:methods.adoc#get[GET], xref:methods.adoc#head[HEAD], xref:methods.adoc#put[PUT], xref:methods.adoc#patch[PATCH], xref:methods.adoc#delete[DELETE], xref:methods.adoc#options[OPTIONS]
...
xref:methods.adoc#put[PUT] only in case it is used to create a new document resource.

|[[http-202]]202 Accepted
|**202 (“Accepted”) must be used to indicate successful start of an asynchronous action**

A 202 response indicates that the client’s request will be handled asynchronously. This response status code tells the client that the request appears valid, but it still may have problems once it’s finally processed. A 202 response is typically used for actions that take a long while to process (see xref:resources-document.adoc#long-running-tasks[Long-running tasks]).
...
|xref:methods.adoc#post[POST], xref:methods.adoc#head[HEAD], xref:methods.adoc#put[PUT], xref:methods.adoc#patch[PATCH], xref:methods.adoc#delete[DELETE], xref:methods.adoc#options[OPTIONS]
...
|All: 
xref:methods.adoc#get[GET], xref:methods.adoc#head[HEAD], xref:methods.adoc#post[POST], xref:methods.adoc#put[PUT], xref:methods.adoc#patch[PATCH], xref:methods.adoc#delete[DELETE], xref:methods.adoc#options[OPTIONS]
...
|xref:methods.adoc#get[GET], xref:methods.adoc#head[HEAD]
...
|All: 
xref:methods.adoc#get[GET], xref:methods.adoc#head[HEAD], xref:methods.adoc#put[PUT], xref:methods.adoc#patch[PATCH], xref:methods.adoc#delete[DELETE], xref:methods.adoc#options[OPTIONS], 
xref:methods.adoc#post[POST] (if parent resource not found)
...
|xref:methods.adoc#post[POST], xref:methods.adoc#put[PUT], xref:methods.adoc#patch[PATCH], xref:methods.adoc#delete[DELETE]
...
|xref:methods.adoc#post[POST], xref:methods.adoc#put[PUT], xref:methods.adoc#patch[PATCH]
