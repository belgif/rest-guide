...
[rule#rule-uri-extens]
.URI extensions
====
The URI **SHOULD NOT contain a file extension**.

A notable exception to this rule is the OpenAPI description (see xref:api_specifications.adoc#doc-resource[doc resource]).
====

Instead, look at how to express xref:http.adoc#media-types[media types] using HTTP headers.
...
|`/search`  | `GET /documents?createdBy=+{emailAddress}+` | `POST /documents/search`
