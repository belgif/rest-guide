...
A document resource is a _singular concept_ that is referring to a business entity or object instance. A document’s state representation typically includes both _fields with values_ and xref:hypermedia.adoc#links[_links_] to other related resources.

*Documents in a collection*

Most document resources are contained in a _parent xref:resources-collection.adoc[collection resource]_ to which a *variable path segment with identity key* is added.
...
|part of collection|`/organizations/012345678901`
a|
```json
{
  "enterpriseNumber": "012345678901",
  "name": "PetShop Incorporated"
}
```
|`/organizations/+{enterpriseNumber}+`
|===

Example in case of a sensitive identifier:

`POST /persons/get` with `ssin` in request body.
...
|singleton with document as parent | `/organizations/012345678901/logo` |
...
`POST /persons/idCardPicture` with `ssin` in request body.
====

== Child resources

A document may have _child resources_ of any type (singleton document, collection or controller) that represent its specific subordinate concepts. When defining child resources, stick to concepts _within the same API_.

In domain modeling, an entity can be modeled as a child of another entity, when its lifecycle depends on the parent, i.e.:

* a child resource cannot be created without a parent
* if the parent is deleted, the child is deleted as well
* it is not possible to change the parent of a child

Occasionally complex documents have so many related resources that putting all these different concepts in a single API becomes unmanageable. For example, an employer document could link to related resources describing employees, debts, taxes, mandates, declarations, risks, etc.
In that case, model the related resources in a separate API and link to the principal resource not by including a link (URL) across APIs (see xref:hypermedia.adoc#rule-hyp-links[[hyp-links\]]) but just by adding the <<section-identifier, identifier>> of the principal resource in the representation.

.Child entities
[rule, child-ent]
====

Interactions with entities that are, in a conceptual data model, children of other entities, can be represented in multiple ways in a REST API.

The *identity* of a child entity can be represented by:

* its own globally unique identifier
* or a composite key (parent id + child id) with the child id only unique within the scope of its parent

If there's a natural business key, whether composite or not, it SHOULD be used instead of introducing a new identifier (also see <<rule-id-choice>>).

Regardless of choice, the identifier of the parent entity SHOULD be included in the full representation of a child and be immutable.

The *path* of a child entity can be designed either as a top-level or child resource path:

|===
|option| preferred when

| top-level resource path
a|
* the parent entity is managed in another API
* resource nesting would become too deep
* clients frequently perform operations on the child entities _without_ knowing the parent id

| child resource path under the parent entity

a|

* the child entity is an integral part of the parent
* the parent resource is managed in the same API
* clients usually perform operations on the child entities _within_ the context of its parent entity
|===
...
Following choices are possible to represent _invitation_ entities:

[cols="1,2,2"]
|===
|identity of child entity | child resource path | top-level resource path
...
In the OpenAPI schema for the identifier, a regular expression may be specified if helpful for input validation or as hint of the structure (e.g. to avoid whitespace or wrong capitalization), but shouldn't be too restrictive in order to be able to evolve the format.
...
|===
| identifier structure | example | OpenAPI schema | considerations
...
Depending on context, the OpenAPI schema enumerate the list of allowed values (see xref:api_specifications.adoc#enum-rule[Enum values rule]).
====
...
* Identifiers that are commonly represented (e.g. when displayed or inputted by a user) with *leading zeros* present SHOULD be represented using a string type. A regular expression SHOULD be specified in the OpenAPI schema to avoid erroneous values (i.e. without leading zeros).
...
** to reference a resource within another one's representation: property name should designate the relation between the resources (see xref:json.adoc#rule-jsn-naming[[jsn-naming\]]); no need to suffix with `id` or `code`
...
* query search parameter: use same name as the property in the JSON resource representation of the response (see xref:resources-collection.adoc#searching[Searching])
* OpenAPI schema: add suffix `Id` or `Code` to distinguish it from the type of the full resource representation
...
OpenAPI schemas (not all are listed for brevity):
```YAML
Order: #<6>
  type: object
  properties:
...
====
```
GET /organizations/{enterpriseNumber} <1>
```
```YAML
{
  "enterpriseNumber": "0123456789", <1>
  "absorbedOrganizations": ["0234567891", "0345678912"], <2>
  "succeededBy": "0456789123", <2>
  "status": "active"
}
```
<1> Standardized business identifier name `enterpriseNumber` is preferred over `id`.
<2> JSON property name designates relation to the other resource. The OpenAPI description declares the expected value to be of schema `EnterpriseNumber`.
====
...
|body
|The representation of the entity, including links to other resources.
a|
...
----
{
  ...
  "enterprise": {
    ...
    "href": "{API}/enterprises/0202239951[/enterprises/0202239951^]"
  }
}
----
...
|xref:statuscodes.adoc#http-400[400]
|Bad request
...
----
|xref:statuscodes.adoc#http-404[404]
|Not Found
|The document resource does not exist.

|===

WARNING: xref:statuscodes.adoc#http-204[204 No content] should not be used when consulting a document.

When the document identifier is sensitive, a POST operation with payload should be used as request:
...
|`/persons/get`
|Consult a person
|body| identifier of the consulted entity
a|
```json
{
  "ssin": "12345678901"
...
The possible responses are the same as for a GET operation, including a `404 Not Found` response code when the document does not exist.
====
...
. use POST on a child resource instead of PATCH if the request does not modify the resource in a way defined by the semantics of the media type.
  See xref:resources-controller.adoc[] for more information.
...
Absent optional values in the request are set to their default value if one is specified in the OpenAPI description.
...
|body
|Full new representation of the resource to persist.
|
...
|body
|Full new representation of the resource to persist, including its (immutable) identifier.
|
|===

The possible responses are the same as for a PUT operation, including a `404 Not Found` response code when the document does not exist.
...
Properties that can be removed, must have `nullable: true` in the OpenAPI 3.0 schema describing the JSON Merge Patch request, which is an exception to xref:json.adoc#rule-jsn-null[[jsn-null\]].
...
|body
|Identifier of the document and the changes to the resource to persist.
|
|===

The possible responses are the same as for a PATCH operation, including a `404 Not Found` response code when the document does not exist. The operation's description should clarify if it provides a full or partial update.
====
...
The cause of failure is represented using an embedded Problem object, as defined in xref:errorhandling.adoc[].
====
