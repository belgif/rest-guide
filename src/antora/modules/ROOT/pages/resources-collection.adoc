...
[rule, col-get]
.API operation to consult a collection
====
An operation to consult a collection resource SHOULD be provided as a `GET` method on the collection resource URI.

When consultation requires input parameters with a complex structure (e.g. extensive search or sorting parameters) or has sensitive data, a `POST` operation on a `/search` controller MAY be used instead of `GET`. This allows to represent the input parameters in a structured manner in a JSON request body without the limitations or exposure of query parameters.
...
=== [[searching]] [[filtering]] Searching

[[rule-col-filter]]
[rule, col-search]
.Searching items in a collection
====
A collection may support xref:reserved.adoc#query-parameters[query parameters] to retrieve a subset of its items:

* a query parameter with the name of a document property, matches items on the specified value
* the query parameter `q` is reserved for a full text search on all the document's content
* other search parameters may be supported, e.g. to look up items within a search period

Unless the API documentation explicitly states otherwise, returned collection items:

* should satisfy ALL search query parameters (AND-logic)
* have to match ANY of the values of a multi-valued search query parameter (OR-logic).
====
...
|OK
|Default response code, also when search matches no items.
| xref:statuscodes.adoc#http-400[400] | Bad Request | Generic error on client side. For example, the syntax of the request is invalid.
| xref:statuscodes.adoc#http-404[404] | Not found | The URI provided cannot be mapped to a resource.
|===
...
| `total` | OPTIONAL | Total number of items across all pages. If search parameters are used, this is the total of the result set, not of the entire collection.
|===
...
By using query parameters, the items to be removed can be selected.
