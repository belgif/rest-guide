...
For extensibility, the JSON body of a RESTful request or response should always be a JSON object instead of an array, string, number, etc. (xref:json.adoc#rule-evo-object[[evo-object\]]).
This way the request or response can always be extended with new properties.
...
[rule#rule-evo-compat]
.Compatible extensions
====
Apply the following rules to evolve RESTful or asynchronous APIs in a backward-compatible way:
...
* Enum values can be added, but not removed when used as input parameters.
* Enum values can be removed when used as output parameters, but not added as clients might not be ready to handle them.
* A new resource variant (with another name) can be created in addition to the old resource variant.
====

Note that while clients must ignore unknown response fields, APIs may refuse requests containing unknown fields (xref:api_specifications.adoc#rule-req-valid[[req-valid\]]).
...
Note that reusable OpenAPI documents are to be versioned as well, as specified in xref:api_specifications.adoc#openapi[OpenAPI].
