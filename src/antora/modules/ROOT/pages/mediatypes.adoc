...
Status code xref:statuscodes.adoc#http-415[415] MUST be used to indicate an unsupported media type in the request payload.

If a resource supports multiple content types through negotation, the response header `Vary: Accept` MUST be added to avoid undesired cache hits (also see xref:performance.adoc#Caching[Caching]).
====
...
* `application/merge-patch+json` for JSON Merge Patch payloads (see xref:resources-document.adoc#partial-update[Partial update])
* `application/problem+json` for Problem Detail for HTTP APIs (RFC 9457) payloads (see xref:errorhandling.adoc[])
