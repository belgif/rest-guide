...
As for xref:http.adoc[], HTTP supports content negotiation for the language used in response content.
...
An API MAY offer to filter the representation to a single language by using the xref:reserved.adoc#query-param-lang[reserved query parameter] `lang`.
====
