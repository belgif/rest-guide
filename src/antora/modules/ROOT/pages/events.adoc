...
For queries: `get`, `search` or a custom method (similar to REST controller)
...
[rule#rule-async-prbl]
====
Each asynchronous interaction (command or query) that provides a reply, SHOULD also provide a problem reply, with a Problem Details object (RFC 9457) as `data`. The same standardized Belgif problem types SHOULD be used as for REST APIs (see xref:errorhandling.adoc[]).
...
The same guidelines as for REST APIs apply, like naming conventions, reuse of components and xref:versioning.adoc[versioning].
