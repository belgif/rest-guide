...
For extensibility, the payload (if any) of a RESTful request or response should always be a JSON object instead of an array, string, number, etc (xref:versioning.adoc#rule-evo-object[]).
This way the request or response can always be extended with new properties ( xref:versioning.adoc#rule-evo-compat[]).
