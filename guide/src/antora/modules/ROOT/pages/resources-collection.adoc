...
CAUTION: A collection resource, like other resources, MUST always return a JSON object as top-level data structure to support extensibility (xref:versioning.adoc#rule-evo-object[]). Do not return a JSON array, because the moment you like to add paging, hypermedia links, etc. your API will break.
...
In this case, the full representations MUST be included in an 'embedded' property as described in xref:performance.adoc#embedding-resources[].
====
