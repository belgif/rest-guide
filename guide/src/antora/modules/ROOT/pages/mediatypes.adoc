...
If a resource supports multiple content types through negotation, the response header `Vary: Accept` MUST be added to avoid undesired cache hits (also see xref:performance.adoc#Caching[]).
====
...
* `application/merge-patch+json` for JSON Merge Patch payloads (see xref:resources-document.adoc#partial-update[])
* `application/problem+json` for Problem Detail for HTTP APIs (RFC 9457) payloads (see xref:errorhandling.adoc#error-handling[])
