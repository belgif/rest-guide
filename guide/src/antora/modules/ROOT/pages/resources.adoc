...
A notable exception to this rule is the OpenAPI description (see xref:api_specifications.adoc#doc-resource[]).
====

Instead, look at how to express xref:mediatypes.adoc#media-types[] using HTTP headers.
...
* xref:resources-document.adoc#Document[]
* xref:resources-collection.adoc#Collection[]
* xref:resources-controller.adoc#Controller[]
