...
|?page=3&pageSize=20
|xref:resources-collection.adoc#Pagination[]
...
|?q=Belgacom
|xref:resources-collection.adoc#Filtering[]

|[[query-param-select]]select
| Filter the resource properties to the ones specified.
| ?select=(name,address)
| xref:resources-document.adoc#document-consult[]
...
|?sort=age&sort=-name
|xref:resources-collection.adoc#Collection[]

| embed
| Request to embed subresource
| ?embed=mainAddress
| xref:performance.adoc#embedding-resources[]

| [[query-param-lang]]lang
| language to filter multi-language descriptions
| ?lang=fr
| xref:internationalization.adoc#multi-language-descriptions[]
...
|Location     | Response | Used in redirection, or when a new resource has been created. |xref:resources-collection.adoc#create-a-new-resource[], xref:methods.adoc#POST[], status codes xref:statuscodes.adoc#http-301[301], xref:statuscodes.adoc#http-303[303] and xref:statuscodes.adoc#http-307[307]
|Accept       | Request  | Media type(s) that is(/are) acceptable for the response. | xref:mediatypes.adoc#media-types[]
|Content-Type	| Request/Response | The Media type of the body of the request. | xref:mediatypes.adoc#media-types[]
|X-HTTP-Method-Override | Request | Override the method specified in the request. | xref:methods.adoc#PATCH[]
|Retry-After  | Response | Suggest amount of time for the client to wait before retrying the request when temporarily unavailable or quota reached | xref:problems/serviceUnavailable.adoc#service-unavailable[], xref:problems/tooManyRequests.adoc#too-many-requests[], xref:problems/tooManyFailedRequests.adoc#too-many-failed-requests[]
|Allow        | Response | Valid methods for a specified resource. | xref:statuscodes.adoc#http-405[]
|ETag         | Request  | Identifier for returned response content |  xref:performance.adoc#conditional-requests[]
|If-None-Match | Response | Return resource if ETag changed | xref:performance.adoc#conditional-requests[]
|Last-Modified  | Request | Time on which resource was last modified | xref:performance.adoc#conditional-requests[]
|If-Modified-Since | Response | Return resource only if changed since specified timestamp | xref:performance.adoc#conditional-requests[]
|Vary | Response | Specifies which request headers change response content | xref:performance.adoc#client-caching-directives[]
|Cache-Control | Response | Indicates HTTP client how to cache responses | xref:performance.adoc#client-caching-directives[]
|===
...
|BelGov-Trace-Id    | Request/Response | Unique ID for tracing purposes, identifying the request or response | xref:tracing.adoc#Tracing[]
|BelGov-Related-Trace-Id | Response | BelGov-Trace-Id value used on related request | xref:tracing.adoc#Tracing[]
|===
...
/doc +
/doc/openapi.yaml +
/doc/openapi.json | API documentation (OpenAPI description and other)  | xref:api_specifications.adoc#doc-resource[]
| /refData | resources representing reference data used in the API (i.e. code lists) | xref:api_specifications.adoc#doc-resource[]
| /health | API health status | xref:health.adoc#health[]
|===
