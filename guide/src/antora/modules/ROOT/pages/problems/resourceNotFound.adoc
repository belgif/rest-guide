...
Note that this problem type is only used when the resource path cannot be resolved. For resources referenced otherwise (i.e. request body, header or query parameter), xref:problems/badRequest.adoc#bad-request[] is returned with a `urn:problem-type:belgif:input-validation:referencedResourceNotFound` issue type.
endif::[]
