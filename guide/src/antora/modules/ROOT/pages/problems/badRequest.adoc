...
ifdef::full-guide[]
(see xref:api_specifications.adoc#rule-req-valid[])
endif::[]
...
ifdef::full-guide[]
This issue type is only used for resource identifiers in header or query parameters or in the request body. For resource identifiers in the request path, a xref:problems/resourceNotFound.adoc#resource-not-found[] Problem is returned instead, with a `404` status code.
endif::[]
|===
