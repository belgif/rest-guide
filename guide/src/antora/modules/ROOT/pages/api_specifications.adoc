...
* `Accept-Language`
* `BelGov-Trace-Id` and `BelGov-Related-Trace-Id` (documented in xref:tracing.adoc#Tracing[])
* `Authorization`
...
As reference data is typically static, consider supporting xref:performance.adoc#Caching[] headers for these resources.
====
...
An example is the description a map of `embedded` resources, as described in xref:performance.adoc#embedding[].
...
When defining a type for an identifier or code, like the above example, the guidelines under xref:resources-document.adoc#Identifier[] apply, even when not used as a URL path parameter of a document resource.
