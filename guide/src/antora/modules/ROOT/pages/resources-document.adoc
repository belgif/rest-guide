...
Most document resources are contained in a _parent xref:resources-collection.adoc#Collection[collection resource]_ to which a *variable path segment with identity key* is added.
...
In that case, model the related resources in a separate API and link to the principal resource not by including a link (URL) across APIs (see xref:hypermedia.adoc#rule-hyp-links[]) but just by adding the <<section-identifier, identifier>> of the principal resource in the representation.
...
Depending on context, the OpenAPI schema enumerate the list of allowed values (see xref:api_specifications.adoc#enum-rule[]).
====
...
** to reference a resource within another one's representation: property name should designate the relation between the resources (see xref:json.adoc#rule-jsn-naming[]); no need to suffix with `id` or `code`
...
* query search parameter: use same name as the property in the JSON resource representation of the response (see xref:resources-collection.adoc#filtering[])
...
. use POST on a child resource instead of PATCH if the request does not modify the resource in a way defined by the semantics of the media type.
  See xref:resources-controller.adoc#Controller[] for more information.
...
Properties that can be removed, must have `nullable: true` in the OpenAPI 3.0 schema describing the JSON Merge Patch request, which is an exception to xref:json.adoc#rule-jsn-null[].
...
The cause of failure is represented using an embedded Problem object, as defined in xref:errorhandling.adoc#error-handling[].
====
