...
Interactions with entities that are, in conceptual data model, children of other entities, can be represented in multiple ways in a REST API.

The *identity* of a child entity can be represented by:

* its own globally unique identifier
* a composite key (parent id + child id) with the child id only unique within the scope of its parent
...
* an _invitation_ could uniquely be identified by `meetingId` and `personId`, as a person can only be invited to a meeting once
...
*(2)* the character used as key separator `:` should be chosen so that it isn't allowed as part of `meetingId` or `personId`
====
...
Depending on context, the OpenAPI schema enumerate the list of allowed values (see <<enum-rule>>).
====
