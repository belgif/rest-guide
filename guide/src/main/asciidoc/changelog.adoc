== Changelog
* 2026-06-12
** new rule <<rule-sens-dat>> on sensitive data in URLs or headers
** new rule <<rule-sch-discr>> split of from <<rule-sch-oneof>> and <<rule-sch-allof>> on URI-reference syntax for discriminator mapping
** added link to example schema to <<Pagination>>
** update <<rule-sch-oneof>>: discriminator mapping should include all `oneOf` subschemas
** new rule <<rule-sec-req>>: security requirements must be consistent with security schemes
** new rule <<rule-opbody-req>>: `required` should be specified explicitly on request bodies
...
* 2026-04-28
** update <<rule-sch-allof>> and <<rule-sch-oneof>>: guidelines on use of `discriminator` and `mapping`
** new rule <<rule-child-ent>> on representation of child entities
** update rule <<rule-oas-descr>> with exception for `title` within `allOf` and `not` inline schemas
** update <<rule-async-svctype>>: `service` should be prefixed with a reverse-DNS name
** new rule <<rule-async-order>> on out-of-order and duplicate messages
** update rule <<rule-async-cespec>>:
...
*** guidelines on context attributes were split off to <<rule-async-ctxt>>
** new rule <<rule-async-generic>> with guidelines on generic endpoints for Asynchronous Messages
** add a list of organizations that endorse this guide (<<Introduction>>)

* 2026-02-16
** new rule <<rule-oas-doc>> on providing documentation in OpenAPI. This replaces the previous `[ext-docs]` rule.
** update rule <<rule-oas-descr>>:
...
** update rule <<rule-sch-oneof>>: changed allowed usage to ensure tooling compatibility (`object` subschemas with `discriminator`, subschemas with only `required` and `enum` subschemas)
** update <<rule-trc-header>>: make UUIDv4 optional for trace ids and recommend randomness

* 2025-11-25
** new guidelines on <<events, Asynchronous messaging and events>>
** new rule <<rule-trace-w3c>> on the use of W3C/OTel tracing only within an organization's trust boundary
* 2025-09-24
