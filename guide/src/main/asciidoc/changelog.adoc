...
** new rule <<rule-sch-discr>> split of from <<rule-sch-oneof>> and <<sch-allof>> on URI-reference syntax for discriminator mapping
** added link to example schema for Pagination to <<Pagination>>
...
** add reference to RFC keywords (MUST, SHOULD, etc), and replace MANDATORY by REQUIRED
