...
====
An API SHOULD refuse requests containing unknown body (JSON), path or query request parameters. A <<Bad Request, Bad Request>> problem response should be returned with an issue of type `urn:problem-type:belgif:input-validation:unknownParameter`.
