...
====
An API SHOULD refuse unknown body (JSON), path or query request parameters with a problem response.
If an operation does process request parameters that are not defined in OpenAPI, its description should explicitly indicate this.
