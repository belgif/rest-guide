...
* The property used as discriminator (i.e. value of `propertyName`) SHOULD be specified by the "parent" schema (declaring `allOf`), and listed as `required`.
...
* an explicit `mapping` of discriminator values to subschemas is RECOMMENDED
...
a|
```YAML
OneOfTwoPropertiesRequiredSchema:
  type: object
