...
* subschemas of `type: object`, when a `discriminator` is also defined. Properties may only be defined within the subschemas. Like for allOf-style polymorphism (<<rule-sch-allof>>), it is RECOMMENDED to provide explicit `mapping` of discriminator values to subschemas.
====
