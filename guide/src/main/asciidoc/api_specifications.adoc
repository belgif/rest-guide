...
.Unknown input
====
An API SHOULD refuse requests containing unknown JSON properties in the payload or unknown query parameters. A <<Bad Request, Bad Request>> problem response should be returned with an issue of type `urn:problem-type:belgif:input-validation:unknownInput`.

If an operation does allow and process request fields that are not defined in OpenAPI, its description should explicitly indicate this.

In specific situations, where a (known) input field is not needed anymore and can be safely ignored:
