...
====
An API SHOULD refuse unknown body (JSON), path or query request parameter request parameters with a problem response.
If an operation does accept request parameters that are not defined in OpenAPI, its description should explicitly indicate this.
...
Refusing unknown body parameters (i.e. JSON bo)
