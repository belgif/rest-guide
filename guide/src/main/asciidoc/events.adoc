...
[rule, async-order]
====
Distributed and concurrent systems make it hard to guarantee exactly-once or order-preserving message delivery. Consumers SHOULD NOT rely on messages being unique and sent in (chronologic) order unless explicitly documented in the API description.

Asynchronous messages SHOULD be designed to allow duplicates and out-of-order processing. A consumer should not need to rely on the order of arrival or any transport-layer information not part of the Asynchronous Message to detect duplicates or an ordered relationship between messages.
...
A consumer that is interested in the current state can then ignore events that are older than the last processed event of each resource. A consumer interested in the history of a resource can use the ordering key to recreate a (partially) ordered sequence of events, and ignore duplicate events with the same ordering key.
