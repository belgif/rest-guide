...
====
When providing a generic asynchronous endpoint for exchange of event types of multiple unrelated business domains, you can use the generic `AsynchronousMessage` schema of https://github.com/belgif/openapi-cloudevents/blob/main/src/main/openapi/cloudevents/v1/cloudevents-v1.yaml[Belgif openapi-cloudevents]. The definitions of specific message types should still be grouped together in a service and documented in another AsyncAPI or OpenAPI description, by extending the `AsynchronousMessageBase` schema.

Additional data outside the `AsynchronousMessage` SHOULD NOT be required to process messages functionally by an application, but MAY support technical aspects during message transfer like routing at batch-level or authentication.

For batch transfers supporting a very large number of messages, like FTP file transfer, the `AsynchronousMessagesBatch` schema SHOULD be used.
====
