...
An *event* is a data record expressing an occurrence and its context. Events are routed from an event producer (the source) to interested event consumers. _This definition is taken from https://github.com/cloudevents/spec/blob/main/cloudevents/spec.md#event)[the CloudEvents Specification]_
...
This guide extends upon the CloudEvents specification, defining additional guidelines and context attributes. It also adds support for asynchronous messages other than events, still using the CloudEvent message format (e.g. an asynchronous request-reply exchange).
