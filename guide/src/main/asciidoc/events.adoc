...
While the combination of `source` and `id` is unique, this might not be sufficient e.g. in a migration scenario to detect duplicates when the same data change is provided in multiple messages in different formats. A separate identifier of the data change may be needed in such case.
